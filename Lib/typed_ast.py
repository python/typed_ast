"""
    ...
    The `typed_ast` module helps Python applications to process trees of the
    Python abstract syntax grammar.  The abstract syntax itself might change
    with each Python release; this module helps to find out programmatically
    what the current grammar looks like and allows modifications of it.  The
    difference between the `typed_ast` module and the builtin `ast` module is
    that `typed_ast` provides PEP 484 type comment information as part of the
    AST.
    ...
    classes all inherit from `typed_ast.AST`.
