...
translabel(grammar *g, label *lb)
{
    ...
    if (lb->lb_type == NAME) {
        ...
        for (i = 0; i < (int)N_TOKENS; i++) {
            if (strcmp(lb->lb_str, _TaParser_TokenNames[i]) == 0) {
                if (Py_DebugFlag)
    ...
    if (lb->lb_type == STRING) {
        if (isalpha(Py_CHARMASK(lb->lb_str[1])) ||
            ...
            int type = (int) TaToken_OneChar(lb->lb_str[1]);
            if (type != OP) {
                lb->lb_type = type;
                free(lb->lb_str);
                lb->lb_str = NULL;
            ...
            int type = (int) TaToken_TwoChars(lb->lb_str[1],
                                       lb->lb_str[2]);
            if (type != OP) {
                lb->lb_type = type;
                free(lb->lb_str);
            ...
            int type = (int) TaToken_ThreeChars(lb->lb_str[1],
                                                lb->lb_str[2],
                                                lb->lb_str[3]);
            if (type != OP) {
                lb->lb_type = type;
